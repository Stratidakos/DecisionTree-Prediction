...
#train
classifier = DecisionTreeClassifier(criterion= 'gini', max_depth=None, max_leaf_nodes= None, min_samples_leaf=50, min_samples_split= 10)
classifier.fit(df_fin_train, y_train)
#predict
y_pred=classifier.predict(df_fin_test)
#writing csv
y_pred_df = pd.DataFrame(y_pred)
y_pred_df.index.name = 'Id'
y_pred_df.columns = ['Label']
y_pred_df.to_csv('decision_tree_pred.csv')
